...
class LimitedSupervisedTaskDistribution(SupervisedTaskDistribution):
    ...
    def sample_task_feed(self, indices_batch) -> FeedList:
    ...
    def epoch(self, **kwargs) -> Iterator[FeedList]:
        ...
        batch_sections = [self.meta_batch_size]
        while batch_sections[-1] < len(self._requests):
            batch_sections.append(batch_sections[-1] + self.meta_batch_size)
        batched_indices = np.split(
            self._rng.permutation(len(self._requests)),
            indices_or_sections=batch_sections[:-1],
        )
        for indices in batched_indices:
            if len(indices) == self.meta_batch_size:
                yield self.sample_task_feed(indices)
